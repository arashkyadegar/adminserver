...
import { ImageEntity, ImageListEntity, ImageListSchema, ImageSchema } from "./imageEntity";
import { ImageRouterClassLogger } from "../logger/imageLogger";
export class ImageRouterClass {
  bus: ImageBus;
  logger: any;
  constructor(b: ImageBus) {
    this.bus = b;
    this.logger = new ImageRouterClassLogger();
  }
  ...
  async updateOne(req, res, next): Promise<any> {
    ...
    let id = req.params.id;
    const imageEntity = req.body as ImageEntity;
    const { error } = ImageSchema.validate(imageEntity);
    if (error) {
    ...
    result = await this.bus.updateOne(id, imageEntity);
  ...
  async createOne(req, res, next): Promise<any> {
    ...
    result = await this.bus.createOne(imageEntity);
  ...
  async createMany(req, res, next): Promise<any> {
    let result;
    const imageEntities = req.body as ImageListEntity;

    const { error } = ImageListSchema.validate(imageEntities);

    if (error) {
      const errorResponse = `validation failed. errors: ${error} `;
      this.logger.logError(errorResponse, "createMany");
      return {
        message: errorResponse,
        status: ResponseStatus.BAD_REQUEST,
      };
    ...
    result = await this.bus.createMany(imageEntities);
...
module.exports = { ImageRouterClass };
